...
try {
  ...
  from `member` mem  join myfavorite mf on (mem.mem_no = mf.mem_no) join singleplaylist spl on ( mf.song_no = spl.song_no) join allplaylist apl on (spl.plist_no=apl.plist_no)  join total_station_music_library tsml on (spl.song_no = tsml.song_no) join `member` mem2 on (mem2.mem_no=tsml.mem_no)
